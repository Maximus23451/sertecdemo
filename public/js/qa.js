...
function showTab(n, el) { document.querySelectorAll('[id^="tab-"]').forEach(t => t.style.display = 'none'); document.querySelectorAll('.nav-item').forEach(x => x.classList.remove('active')); document.getElementById('tab-' + n).style.display = ''; el.classList.add('active'); }
function esc(s) { return API.escHtml(s); }

function populateSelect() {
  const sel = document.getElementById('questionSelect');
  sel.innerHTML = questions.length ? questions.map(q => `<option value="${q.id}">${esc(q.text)}</option>`).join('') : '<option value="">— No questions available —</option>';
...
function renderPending() {
  ...
  if (pendingQ) { el.classList.add('show'); tx.textContent = pendingQ.text; } else { el.classList.remove('show'); }
}
