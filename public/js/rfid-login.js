...
function initSwipe() {
  ...
  const THUMB_W = 42; let dragging = false, startX = 0, startLeft = 4;
  function maxLeft() { return track.offsetWidth - THUMB_W - 4; }
  function onStart(e) {
    if (swipeDone) return; dragging = true;
    startX = e.touches ? e.touches[0].clientX : e.clientX;
    startLeft = parseInt(thumb.style.left || '4', 10);
    thumb.style.transition = 'none'; fill.style.transition = 'none'; e.preventDefault();
  }
  function onMove(e) {
    if (!dragging || swipeDone) return;
    const cx = e.touches ? e.touches[0].clientX : e.clientX;
    const nl = Math.min(Math.max(startLeft + cx - startX, 4), maxLeft());
    thumb.style.left = nl + 'px';
    fill.style.width = ((nl - 4) / (maxLeft() - 4) * 100) + '%';
    e.preventDefault();
  }
  async function onEnd() {
    if (!dragging) return; dragging = false;
    const cur = parseInt(thumb.style.left || '4', 10);
    const pct = (cur - 4) / (maxLeft() - 4) * 100;
    if (pct >= 88) {
      thumb.style.transition = 'left 0.2s ease'; thumb.style.left = maxLeft() + 'px';
      ...
      try {
        const user = await API.login(document.getElementById('operatorSelect').value, 'demo123');
        API.setUser(user);
        setTimeout(() => { location.href = '/operator.html'; }, 500);
      } catch (e) {
        document.getElementById('rfidStatus').className = 'rfid-status err';
        document.getElementById('rfidStatus').textContent = 'Login failed: ' + e.message;
        swipeDone = false; track.classList.remove('done');
        thumb.style.transition = 'left 0.25s ease'; thumb.style.left = '4px';
        fill.style.transition  = 'width 0.25s ease'; fill.style.width = '0%';
      }
    } else {
      thumb.style.transition = 'left 0.25s ease'; thumb.style.left = '4px';
      fill.style.transition  = 'width 0.25s ease'; fill.style.width = '0%';
