...
API.subscribe({
  ...
  docs:      d => { docs = d; renderDocs(); renderStats(); },
});
...
function renderQList() {
  ...
  if (!questions.length) {
    el.innerHTML = '<div class="empty-hint">No questions yet.</div>';
    ov.innerHTML = '<div class="empty-hint">No questions configured</div>'; return;
  }
  const rows = questions.map(q => `
    <div class="q-item">
      <div class="q-text">${esc(q.text)}</div>
      <div class="q-freq">${q.freq}</div>
      <button class="btn-icon del" onclick="delQuestion('${q.id}')" title="Delete">
        <svg viewBox="0 0 24 24"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v6M14 11v6"/></svg>
      </button>
    </div>`).join('');
  el.innerHTML = rows;
  ov.innerHTML = questions.map(q => `
    <div class="q-item">
      <div class="q-text">${esc(q.text)}</div>
      <div class="q-freq">${q.freq}</div>
    </div>`).join('');
